...
public class TreeViewerComparatorTest extends ViewerComparatorTest {
	...
	protected class TeamModelTreeContentProvider extends
			TeamModelContentProvider implements
			ITreeContentProvider<Object, Object> {
	...
	public void testViewerSorterInsertElement() {
		...
		String[][] expected = { TEAM3_SORTED, TEAM2_SORTED,
				TEAM1_SORTED_WITH_INSERT };
		assertSortedResult(expected);
	}

	public void testViewerComparator() {
		comparatorTestViewer
				.setComparator(new ViewerComparator<Object, Object>());
		getTreeViewer().expandAll();
		String[][] expected = { TEAM3_SORTED, TEAM2_SORTED, TEAM1_SORTED };
		assertSortedResult(expected);
	}
	...
	private void assertSortedResult(String[][] resultArrays) {
		...
		assertEquals("Number of root items in tree not correct (actual="
				+ rootItems.length + ")", 3, rootItems.length);
		TreeItem item = rootItems[0];
		assertEquals("Item not expected.  actual=" + item.getText()
				+ " expected=" + CORE, CORE, item.getText());
		item = rootItems[1];
		assertEquals("Item not expected.  actual=" + item.getText()
				+ " expected=" + RUNTIME, RUNTIME, item.getText());
		item = rootItems[2];
		assertEquals("Item not expected.  actual=" + item.getText()
				+ " expected=" + UI, UI, item.getText());
	...
	protected StructuredViewer createViewer(Composite parent) {
		Tree tree = new Tree(fShell, SWT.SINGLE | SWT.H_SCROLL | SWT.V_SCROLL
				| SWT.BORDER);
