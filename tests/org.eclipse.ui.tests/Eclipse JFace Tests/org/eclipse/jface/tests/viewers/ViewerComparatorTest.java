...
public abstract class ViewerComparatorTest extends ViewerTestCase {
	...
	protected String[] TEAM1 = { "Karice", "Tod", "Eric", "Paul", "Mike",
			"Michael", "Andrea", "Kim", "Boris", "Susan" };
	protected String[] TEAM1_SORTED = { "Andrea", "Boris", "Eric", "Karice",
	...
	protected class Team {
		...
		public void addMember(String person) {
			...
			members = newMembers;
			newMembers = null;
			fireModelChanged(new ComparatorModelChange(TestModelChange.INSERT,
					this, newMember));
		}
	 ...
	 * content provider
	 */
	protected class TeamModelContentProvider implements
			IComparatorModelListener,
			IStructuredContentProvider<Object, Object> {
		@Override
		...
		protected void doInsert(ComparatorModelChange change) {
			if (comparatorTestViewer instanceof ListViewer) {
				if (change.getParent() != null
						&& change.getParent().equals(
								comparatorTestViewer.getInput())) {
					((ListViewer<Object, Object>) comparatorTestViewer)
							.add(change.getChildren());
				}
			} else if (comparatorTestViewer instanceof TableViewer) {
				if (change.getParent() != null
						&& change.getParent().equals(
								comparatorTestViewer.getInput())) {
					((TableViewer<Object, Object>) comparatorTestViewer)
							.add(change.getChildren());
				}
			} else if (comparatorTestViewer instanceof AbstractTreeViewer) {
				((AbstractTreeViewer<Object, Object>) comparatorTestViewer)
						.add(change.getParent(), change.getChildren());
			} else if (comparatorTestViewer instanceof ComboViewer) {
				((ComboViewer<Object, Object>) comparatorTestViewer).add(change
						.getChildren());
			} else {
		...
		protected void doRemove(ComparatorModelChange change) {
			if (comparatorTestViewer instanceof ListViewer) {
				...
				((ComboViewer<Object, Object>) comparatorTestViewer)
						.remove(change.getChildren());
			} else {
				Assert.isTrue(false, "Unknown kind of viewer");
			}
